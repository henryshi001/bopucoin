...
const char * const BITCOIN_CONF_FILENAME = "bopucoin.conf";
const char * const BITCOIN_PID_FILENAME = "bopucoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bopucoin" is a composite category enabling all BopuCoin-related debug output
            if(ptrCategory->count(string("bopucoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BopuCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BopuCoin
    // Mac: ~/Library/Application Support/BopuCoin
    // Unix: ~/.bopucoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BopuCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BopuCoin";
#else
    // Unix
    return pathRet / ".bopucoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bopucoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bopucoin.conf
