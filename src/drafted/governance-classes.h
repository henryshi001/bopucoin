 ...
 *
 *   // network
 *   CBopuCoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.bopucoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CBopuCoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CBopuCoinNetwork(UniValue objIn)
    {
...
// // can be under: BopuCoinNetwork
// //   -- signature requirements : Key1(User)
// class CBopuCoinNetworkVariable : public CGovernanceObject
// {
// private:
