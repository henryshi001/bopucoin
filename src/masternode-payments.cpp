...
*
*   Why is this needed?
*   - In BopuCoin some blocks are superblocks, which output much higher amounts of coins
...
CAmount GetDevelopmentBudgetPayment(int nHeight, Consensus::Params params) {
    return nHeight >= 5040 ? 0.02 * GetBlockSubsidy(nHeight, params) : 0;
}
...
{
    if(fLiteMode) return; // disable all BopuCoin specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f BPC\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
